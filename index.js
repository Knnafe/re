...
bot.on("ready", async () => {
  ...
  bot.user.setActivity(`rematch.co.il | ?help  `, {type: "WATCHING"});

});

bot.on('guildMemberAdd', member => {
  let channel = member.guild.channels.find('name', 'שלום-ולהתראות');
  let memberavatar = member.user.avatarURL
      if (!channel) return;
      let embed = new Discord.RichEmbed()
      .setColor('RANDOM')
      .setThumbnail(memberavatar)
      .addField(':bust_in_silhouette: | שם : ', `${member}`)
      .addField(':microphone2: | ברוך הבא!', `ברוך הבא לשרת, ${member}`)
      .addField(':id: | User :', "**[" + `${member.id}` + "]**")
      .addField(':family_mwgb: | אתה המשתמש מספר', `${member.guild.memberCount}`)
...
bot.on('guildMemberRemove', member => {
  ...
  let memberavatar = member.user.avatarURL
      ...
      .addField('שם:', `${member}`)
      .addField('עזב', ';(')
      .addField('ביי חבר :cry:', 'נתגעגע!')
      .addField('לשרת יש', `${member.guild.memberCount}` + " משתמשים")
      .setFooter(`${member.guild.name}`)
      .setTimestamp()
      channel.sendEmbed(embed);
});
